...
import { precisionMinBonus } from "../settings"
import Svg, { G, Path } from 'react-native-svg'
import { getLang } from './OptionsPage'

const GameOver = ({ hits, shootsCount, restart, backHome }) => {
	const [best, setBest] = useState(0)
	const precision = Number.parseFloat((hits / shootsCount).toFixed(2))
	const bonus = precision > precisionMinBonus ? precision - precisionMinBonus : 0
	const score = Math.round(hits * (1 + bonus))
		...
		async function updateStats() {
			try {
				...
				if (prevStats) {
					...
					const bestScore = score > prevStats.bestScore ? score : prevStats.bestScore
					const bestPrecision = precision > prevStats.bestPrecision ? precision : prevStats.bestPrecision

					setBest(bestScore)
					await AsyncStorage.setItem("stats", JSON.stringify({
						bestScore,
						bestPrecision,
						nbPlay: prevStats.nbPlay + 1,
						totalHits: prevStats.totalHits + hits,
						averageScore: (prevStats.nbPlay * prevStats.averageScore + score) / (prevStats.nbPlay + 1),
						averagePrecision: (prevStats.nbPlay * prevStats.averagePrecision + precision) / (prevStats.nbPlay + 1)
					}))
				} else {
					setBest(score)
					await AsyncStorage.setItem("stats", JSON.stringify({
